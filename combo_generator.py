...
class ComboGenerator:
    def __init__(self):
        ...
        self.base_odds = {"1": {"min": 1.15, "max": 1.80}, "1X": {"min": 1.05, "max": 1.25}, "+2.5": {"min": 1.40, "max": 2.00}, "BTTS_YES": {"min": 1.55, "max": 2.10}, "BTTS_NO": {"min": 1.50, "max": 2.00}}
    ...
    def get_match_predictions(self, match):
        ...
        for ptype, data in analysis["predictions"].items():
            if data["confidence"] >= self.min_confidence:
                valid.append({"match_id": match["id"], "home_team": match["home_team"], "away_team": match["away_team"], "league": match["league"], "type": ptype, "type_name": self.get_prediction_name(ptype), "confidence": data["confidence"], "estimated_odds": self.estimate_odds(ptype, data["confidence"])})
        return valid
