...
def api_generate():
    try:
        ...
        all_analyses = generator.analyzer.analyze_multiple_matches(upcoming)
        print(f"Analyses IA recues: {len(all_analyses)}")
        
        # Construire les predictions
        all_preds = []
        for i, match in enumerate(upcoming):
            if i < len(all_analyses):
                ia = all_analyses[i]
                analysis = generator.analyzer.analyze_match_from_ia(match["home_team"], match["away_team"], ia)
            else:
                analysis = generator.analyzer.analyze_match(match["home_team"], match["away_team"])
            
            preds = generator.get_predictions_from_analysis(match, analysis)
